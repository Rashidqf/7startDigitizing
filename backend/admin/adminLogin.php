...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt->bind_param("s", $email);  // Check by email
    $stmt->execute();
    $stmt->store_result();
    ...
    if (password_verify($password, $storedPassword)) {
        ...
        $_SESSION['is_admin'] = $isAdmin;  // Store the admin status

        // Respond with login success and admin status
        echo json_encode([
            'success' => true,
            'message' => 'Login successful!',
            'user_id' => $userId,
            'is_admin' => $isAdmin,  // Send admin status in the response
        ]);
    } else {
        // Invalid password
        echo json_encode(['success' => false, 'message' => 'Invalid email or password.']);
